"""
NFL Data Feature Engineering — LSTM-ready (last 15 frames history)
"""
...
training_df = pd.read_csv(
    r"C:\Users\thoml\OneDrive\Documenten\Uhasselt master jaar 2 sem 1\Machine learning\Kaggle competition\training_data.csv"
)
...
# Mirror longitudinal velocity
df.loc[mask_left, 'vx'] = -df.loc[mask_left, 'vx']

# Recompute direction from vx, vy (now canonicalized)
...
eps = 1e-6
history_len = 15   # last 15 frames for LSTM-style history

# Player body / age features
...
# Release frame per player (last input frame)
...
release_df['ball_angle'] = np.arctan2(release_df['ball_dy'], release_df['ball_dx'])

release_df['dir_rad'] = np.deg2rad(release_df['dir'])
release_df['angle_diff_ball'] = (
    (release_df['ball_angle'] - release_df['dir_rad'] + np.pi) % (2 * np.pi)
) - np.pi
...
release_df['heading_alignment'] = (
    release_df['vx'] * release_df['ball_dx'] + release_df['vy'] * release_df['ball_dy']
) / (speed_norm * release_df['distance_to_ball'].replace(0, eps))
...
release_df['projection_distance_to_ball'] = np.hypot(
    release_df['projection_x'] - release_df['ball_land_x'],
    release_df['projection_y'] - release_df['ball_land_y']
)

release_df['endzone_distance'] = 120.0 - release_df['x']

# Deltas over the last history_len frames (LSTM history)
df_sorted = df.sort_values(['game_id', 'play_id', 'nfl_id', 'frame_id'])

def last_deltas(g):
    g = g.tail(history_len)
...
if len(defenders) > 0:
    defenders['dist_def_to_ball'] = np.hypot(
        defenders['x'] - defenders['ball_land_x'],
        defenders['y'] - defenders['ball_land_y']
    )
    defence_min = defenders.groupby(['game_id', 'play_id'])['dist_def_to_ball'].min().rename(
        'min_defender_dist_to_landing'
    ).reset_index()
    defence_count = defenders.assign(
        within_R=(defenders['dist_def_to_ball'] <= defenders_radius)
    ).groupby(['game_id', 'play_id'])['within_R'].sum().rename(
        f'n_def_within_{int(defenders_radius)}'
    ).reset_index()
else:
...
release_df[f'n_def_within_{int(defenders_radius)}'] = (
    release_df[f'n_def_within_{int(defenders_radius)}'].fillna(0).astype(int)
)

# Nearest defender + player stats
...
# Velocity, momentum, sideline distances
...
features = [
    ...
    'ball_dx', 'ball_dy', 'ball_land_x', 'ball_land_y', 'distance_to_ball', 'ball_angle',
    'angle_diff_ball', 'eta_to_ball',
    ...
    'player_height_inches', 'player_weight', 'player_birth_date', 'player_age',
...
release_df.to_csv("features_data_lstm_15frames.csv", index=False)
